...
class KMeans {
	...
	void pickPointInitial(double X[][], double cclusters[][], int n) {
		int i, j, k, h;
		double tmp, maxd;
		double maxDis[] = new double[X.length];
		int pos = (int)(Randomize.Rand()*X.length); //(int)(rdm.nextDouble()*X.length);
		for(j = 0; j < X[pos].length; j++) {
			cclusters[0][j] = X[pos][j];
		}
		for(i = 0; i < X.length; i++) {
			maxDis[i] = distance(X[i], cclusters[0]);
		}
		for(k = 1; k < n; k++) {
			maxd = 0;
			for(i = 0; i < X.length; i++) {
			...
			for(j = 0; j < X[pos].length; j++) {
				cclusters[k][j] = X[pos][j];
			}
			if(k < n - 1) {
				for(i = 0; i < X.length; i++) {
					tmp = distance(X[i], cclusters[k]);
					if(tmp < maxDis[i]) {
						maxDis[i] = tmp;
					}
				}
...
*/ 	
	public KMeans(double [][]X, int nclusters) {// Randomize vrand) {
		...
		pickPointInitial(X, cclusters, nclusters);
...
*/
	public void clsByKmeans(double [][]X, int nclusters, int cluster[]) {
		KMeans KM= new KMeans(X,nclusters);//,vrand);
        for (int i=0;i<X.length;i++) {
            cluster[i]=KM.nearestCentroid(X[i]);
        }
	}

	private static double[][] myReadFile() {
		String f = "D:/keel/dist/data/ecoli4/ecoli4-5-2tra.dat";
		double res[][];
        ...
        try {
            ...
            while ((tempString = reader.readLine()) != null) { // 显示行号
                if(tempString.length()>0) {
	                ...
	                } else if (tempString.startsWith("@inputs")) {
	                	nattr = tempString.split(",").length;
	                }
                }
            }
            reader.close();
            res = new double[slist.size()][nattr];
            String tmp [];
            for (int i = 0; i < slist.size(); i++) {
            	tempString = slist.get(i);
            	tmp = tempString.split(",");
            	for (int j = 0; j < nattr; j++) {
            		res[i][j] = Double.valueOf(tmp[j]);
            	}
            }
        } catch (IOException e) {
            e.printStackTrace();
	...
	public static void main(String args[]) {
		String path = "src/conf/crossValEH/config0s0.txt"; //crossValEH/config_eoeusbI2";config_eoeusbI
		boolean tty=false;
		ProcessConfig pc=new ProcessConfig();
		System.out.println("Reading configuration file: "+path);
		if (pc.fileProcess(path)<0) return;
        ...
        System.out.println(KM.clusters);
